function [mrsiDataLR_tkk,LipidFree_img,Lipid_Vol,VariableBeta]  = L2SVDLipidSuppression( mrsiData_tkk, mrsiReconParams, Nbasis, Nfit, Beta )
...
img_combined_L2 = Data_rrf.*repmat(meta_mask,[1 1 N(end)]);
...
for comp = 1:Nfit
      ...
      V(:,comp) = (squeeze(Lipid_inv) * Vorig(:,comp));
       V2(:,comp) = Lipid *Lipid' * Vorig(:,comp);
      plot(1:N(3),squeeze(real(V(:,comp))),1:N(3),squeeze(real(Vorig(:,comp))),1:N(3),squeeze(real(V2(:,comp))),1:N(3),squeeze(real(Vorig(:,comp)-V2(:,comp))));
  end
